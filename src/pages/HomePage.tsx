    ...
    <div className="min-h-screen bg-zinc-950 text-white">
      ...
      <section className="relative min-h-[70vh] sm:min-h-screen flex items-center justify-center overflow-hidden pt-16">
        ...
        <div className="relative z-10 text-center px-4 sm:px-6 max-w-5xl mx-auto">
          ...
          <p className="text-zinc-400 text-base sm:text-xl max-w-2xl mx-auto mb-6 sm:mb-10 leading-relaxed">
            The most advanced iPhone ever â€” now at an unprecedented price. Available exclusively through Nover for US residents only.
          </p>
      ...
      <section className="border-t border-zinc-800/50 py-12 sm:py-16">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 grid grid-cols-2 sm:grid-cols-4 gap-6 sm:gap-8 text-center">
          {[
            { icon: 'ðŸ‡ºðŸ‡¸', title: 'US Only', desc: 'Exclusive to United States' },
            { icon: 'ðŸ“¦', title: 'Free Shipping', desc: 'Express delivery included' },
            { icon: 'ðŸ”’', title: 'Secure Checkout', desc: '256-bit SSL encryption' },
            { icon: 'âš¡', title: 'Fast Dispatch', desc: 'Ships within 24 hours' },
          ].map((item) => (
      ...
      <footer className="border-t border-zinc-800/50 py-8 text-center">
        <p className="text-zinc-600 text-xs">Â© 2026 Nover. All rights reserved. This is an exclusive promotional event.</p>
      </footer>
    </div>
  );
}
...
function ProductCard({ product }: { product: typeof products[0] }) {
  ...
  // (No auto-slide â€” manual swipe/arrows only)
    ...
    <div className="mb-16 sm:mb-24 last:mb-0 scroll-mt-20" id={product.id}>
      <div className="bg-zinc-900/50 rounded-2xl sm:rounded-3xl border border-zinc-800/50 overflow-hidden">
          ...
          <div className="p-6 sm:p-8 lg:p-10 flex flex-col">
            ...
            <div className="flex items-center gap-2 mb-4">
              <div className="flex items-center gap-0.5">
                {[...Array(5)].map((_, i) => (
                  <span key={i} className={`text-sm ${i < Math.floor(product.rating) ? 'text-amber-400' : 'text-zinc-700'}`}>â˜…</span>
                ))}
              </div>
            ...
            <div className="mb-5">
              <label className="text-sm font-semibold text-zinc-300 mb-2 block">
                Color â€” <span className="text-white">{selectedColor}</span>
              </label>
              <div className="flex gap-3">
                {product.colors.map((color) => (
                  ...
                  >
                    {selectedColor === color.name && (
                      <span className={`text-xs font-bold ${color.value === '#E8E8E8' ? 'text-zinc-800' : 'text-white'}`}>âœ“</span>
                    )}
                  </button>
                ))}
              </div>
            ...
            <div className="mb-5">
              <label className="text-sm font-semibold text-zinc-300 mb-2 block">
                Storage â€” <span className="text-white">{selectedStorage}</span>
              </label>
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
              {product.specs.slice(0, 6).map((spec) => (
                <div key={spec.label} className="p-3 rounded-lg bg-zinc-800/40 border border-zinc-800/50">
                  ...
                  <p className="text-white text-xs font-medium leading-snug">{spec.value.length > 40 ? spec.value.slice(0, 40) + 'â€¦' : spec.value}</p>
                </div>
              ))}
            </div>
            ...
            >
              Order Now â€” ${adjustedPrice.toFixed(2)}
            </button>
          </div>
        </div>
        ...
        <div className="px-6 sm:px-8 lg:px-10 pb-6 sm:pb-8">
          ...
          <p className="text-zinc-600 text-xs mt-3">Source: GSMArena â€” specifications may vary by region</p>
        </div>
      </div>
    </div>
  );
...
function ProductCard({ product }: { product: typeof products[0] }) {
  ...
  // FIX: REMOVED the useEffect that contained .scrollIntoView() 
  // This was the "ghost" causing the page to jump on load.
